        ...
        $('.login-form input').keypress(function(e) {
            if (e.which == 13) {
                if ($('.login-form').validate().form()) {
                    ...
                    logData.password = hex_md5(logData.password);
                    loginCheck(logData);
                }
                return false;
            }
...
function loginCheckEnd(flg, result){
    if(flg){
        if (result && result.code == SUCCESS){
            ...
            $('.alert-danger', $('.login-form')).show();
            $('.alert-danger span', $('.login-form')).text(result.message);
        }
    }else{
        App.unblockUI('.login-container');
        userPowerDataGet();
...
function loginSubmit(data){
    console.info(data);
    var form = $(".login-form");
    form[0].action = "/back/main";
    form.append($("<input/>").attr("type", "hidden").attr("name", "username").attr("value", data.userid));
